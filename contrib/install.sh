...
if [[ -e /etc/os-release ]]; then
    ...
    if [[ $INSTALL_FAAFO -eq 1 ]]; then
        if [[ $ID = 'ubuntu' || $ID = 'debian' ]]; then
            ...
            sudo yum install -y python-devel python-pip supervisor git zlib-devel mariadb-devel gcc
...
priority=10"
        ...
        if [[ $ID = 'ubuntu' || $ID = 'debian' ]]; then
            ...
            echo "$faafo_api" | sudo tee -a /etc/supervisor.d/faafo.ini
        else
            echo "error: distribution $ID not supported"
            exit 1
        fi
...
priority=20"
        ...
        if [[ $ID = 'ubuntu' || $ID = 'debian' ]]; then
            ...
            echo "$faafo_worker" | sudo tee -a /etc/supervisor.d/faafo.ini
        else
            echo "error: distribution $ID not supported"
            exit 1
        fi
